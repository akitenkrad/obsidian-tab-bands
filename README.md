...
(`src/i18n.ts`)．本 README の表記は日本語表示のもの．
...
バンド末尾のメンバーをアクティブにしたまま新しいタブを開いた場合も同じ (Chrome と同じ挙動)．
どちらも**そのとき新しく開かれたタブ**に限る．既存タブを動かしただけでは参加しない
(位置だけから推論すると誤吸収が起きるため — 「実装メモ」参照)．
...
- **設定項目は持たない．** 既定色は未使用色の自動割り当て，チップ名の幅は CSS 固定
  (`max-width: 12ch`)，折りたたみ時の書式は「バンド名 (N)」固定．調整したい場面が
  実際に出てきたら，そのときに Obsidian の設定画面 (`PluginSettingTab`) へ出す
  (2026-08-29)．
